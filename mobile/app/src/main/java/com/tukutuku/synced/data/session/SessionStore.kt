...
class SessionStore @Inject constructor(private val dataStore: DataStore<Preferences>) {
    companion object { private val TOKEN=stringPreferencesKey("synced_access_token_v1"); private val USER=stringPreferencesKey("synced_user_id_v1"); private val EMAIL=stringPreferencesKey("synced_user_email_v1"); private val NAME=stringPreferencesKey("synced_user_name_v1"); private val WALLET=stringPreferencesKey("synced_wallet_id_v1"); private val SMS_LAST=stringPreferencesKey("synced_sms_last_timestamp_v1") }
    ...
    suspend fun clear() = dataStore.edit { p -> p.remove(TOKEN); p.remove(USER); p.remove(EMAIL); p.remove(NAME); p.remove(WALLET) }
