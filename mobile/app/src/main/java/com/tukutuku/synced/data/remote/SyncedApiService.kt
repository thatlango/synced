...
interface SyncedApiService {
    ...
    @POST("baskets/{id}/contributions") suspend fun contribute(
        @Path("id") id: String,
        @Body body: ContributionRequest,
    ): ApiEnvelope<JsonElementEnvelope>
    ...
    @GET("analytics/personal/trends") suspend fun personalTrends(
        @Query("months") months: Int = 6,
    ): ApiEnvelope<List<MonthlyTrend>>
    @GET("analytics/household/{id}") suspend fun householdAnalytics(
        @Path("id") householdId: String,
    ): ApiEnvelope<HouseholdAnalytics>

    @GET("forecasts/personal") suspend fun personalForecast(): ApiEnvelope<PersonalForecast>

    @GET("bills") suspend fun bills(
        @Query("includePaid") includePaid: Boolean = false,
    ): ApiEnvelope<List<Bill>>
    ...
    @POST("ingestion/sms/candidates/bulk") suspend fun ingestSmsCandidates(
        @Body body: SmsCandidateBulkRequest,
    ): ApiEnvelope<SmsCandidateBulkResult>
}

@kotlinx.serialization.Serializable
data class JsonElementEnvelope(
    val contribution: kotlinx.serialization.json.JsonElement? = null,
